...
exports.isValidNewsUrl = function (url, verbose = false) {
  ...
  if (_.endsWith(urlPath, '/')) { urlPath = urlPath.slice(0, -1); }

  if (_.startsWith(urlPath, '/')) { urlPath = urlPath.slice(1); }


  const pathChuncks = _url._splitPath(urlPath);
  ...
  const tldData = tldParser(url);
  const domainName = _.pull(_.split(tldData.domain, '.'), tldData.tld)[0];
  const urlSlug = _.last(pathChuncks);
