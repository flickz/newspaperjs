...
exports._followUrlFormat = function (url) {
  const regEx = /(http|https):\/\/(w{3}\.)?\w+\.(\w{3})/;
  if (regEx.test(url)) {
    return true;
  }
  return false;
...
exports._validFileType = function (pathChuncks) {
  const fileType = _this._getFileType(pathChuncks);
...
exports._getFileType = function (pathChuncks) {
  const lastChunk = _.last(pathChuncks);
...
exports._urlHasDate = function (url) {
  /*
    ...
    */
  const regEx = /([\./\-_]{0,1}(19|20)\d{2})[\./\-_]{0,1}(([0-3]{0,1}[0-9][\./\-_])|(\w{3,5}[\./\-_]))([0-3]{0,1}[0-9][\./\-]{0,1})?/;
  if (regEx.test(url)) {
    return true;
  }
  return false;
...
exports._hasNewsSlug = function (urlSlug, domainName) {
  const dashCount = occurrences(urlSlug, '-');
  const underscoreCount = occurrences(urlSlug, '_');

  if (dashCount > 4 || underscoreCount > 4) {
    if (dashCount >= underscoreCount) {
      // check if the domain name is in slug
      if (_.indexOf(_.split(urlSlug, '-'), domainName) == -1) { return true; }
    }
    if (underscoreCount > dashCount) {
      // check if the domain name is in slug
      if (_.indexOf(_.split(urlSlug, '_'), domainName) == -1) { return true; }
    }
  }
  return false;
};
...
exports._hasUrlQuery = function (url) {
  const parsedUrl = urlPaser.parse(url);
...
exports._removeArgs = function (url, keep_params = []) {
  const parsedUrl = urlPaser.parse(url, true);
  const filtered_query = ((query) => {
