...
exports._getCategoryUrls = function (sourceUrl, links, categories = []) {
  ...
  for (let i = 0; i < links.length; i++) {
    if (typeof links[i] === 'string') {
      const domain = url.getDomainName(links[i]);
      const protocol = url.getProtocol(links[i]);
      let path = url.getPath(links[i]);
      const parsedSourceUrl = tldParser(sourceUrl);
      ...
      if (domain) {
        const parsedlink = tldParser(links[i]);
